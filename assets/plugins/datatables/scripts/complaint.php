...
$joinQuery = 'FROM `comments` AS `c` JOIN `nodes` AS `n` ON (`c`.`comment_id` = `n`.`node_id`) JOIN `nodes` AS `np` ON (`c`.`parent_id` = `np`.`node_id`) JOIN `users` AS `u` ON (`u`.`user_id` = `n`.`user_id`)';
$extraWhere = "`n`.`module` = 'comment' AND `c`.`parent_id` = '45'";
