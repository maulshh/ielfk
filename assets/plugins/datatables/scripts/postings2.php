...
$joinQuery = 'FROM `posts` AS `p`
				JOIN `post_type` AS `pt` ON (`p`.`post_type_id` = `pt`.`post_type_id`)
				JOIN `nodes` AS `n` ON (`p`.`node_id` = `n`.`node_id`)
				JOIN `users` AS `u` ON (`u`.`user_id` = `n`.`user_id`)
				JOIN `tags` AS `t` ON (`p`.`node_id` = `t`.`post_id`)
				JOIN `comments` AS `c` ON (`c`.`post_id` = `p`.`node_id`)';
$extraWhere = "`n`.`module` = 'post' AND `pt`.`post_name` = ".$_GET['post'];
